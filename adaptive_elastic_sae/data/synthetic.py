...
class SpikedDataConfig:
	...
	k_sparse: int = 16  # Number of 'true' active features for ground truth (sparsity level)
...
class SpikedDataGenerator:
	...
	def __init__(
		...
		if not (1 <= k_sparse <= d_dict):
			raise ValueError(f"k_sparse must be in [1, d_dict], got {k_sparse} for d_dict={d_dict}")
		if noise_std < 0.0:
	...
	def dictionary_stats(self, samples: int = 100) -> dict[str, float]:
		...
		for _ in range(samples):
			idx = torch.randperm(self.d_dict, generator=self._rng, device=self.device)[: self.k_sparse]
			d_a = self.D[:, idx]
			g_a = d_a.T @ d_a
		...
		cond_nums_t = torch.tensor(active_cond_nums, dtype=self.dtype, device=self.device)
		...
		return {
			...
			"expected_active_condition_number_std": cond_nums_t.std(unbiased=False).item(),
